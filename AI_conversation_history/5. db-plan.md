...
- `travelers` **integer** NULL, `CHECK (travelers IS NULL OR travelers > 0)`
- `created_at` **timestamptz**, `DEFAULT now()`, **NOT NULL**
...
- `output_raw` **jsonb** NULL _(raw model/LLM response)_
...
- `content` **jsonb**, **NOT NULL** _(canonical plan content; structured JSON used by the UI)_
- `content_md` **text** NULL _(optional markdown for rendering/editing)_
- `status` **plan_status** (`generated|accepted|rejected`), `DEFAULT 'generated'`, **NOT NULL**
...
> Co-ownership enforcement:  
> Composite FK `FOREIGN KEY (trip_request_id, user_id) REFERENCES trip_requests(id, user_id) ON DELETE CASCADE`  
> Composite FK `FOREIGN KEY (plan_generation_id, user_id) REFERENCES plan_generations(id, user_id) ON DELETE RESTRICT`  
...
CREATE TYPE plan_status            AS ENUM ('generated','accepted','rejected');
...
- `ON DELETE CASCADE` on most dependencies from `users` and `trip_requests` (simplifies cleanup in MVP).  
  `plans(plan_generation_id)` uses `ON DELETE RESTRICT` to avoid accidental plan removal when pruning generation logs.
...
- **`body_md` length (1k–10k chars)** is required by PRD but, per MVP decisions, enforced in the application. If needed, add:  
  `CHECK (char_length(body_md) BETWEEN 1000 AND 10000)`.
