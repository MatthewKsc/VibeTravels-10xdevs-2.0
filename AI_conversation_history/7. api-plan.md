...
- **POST** `/notes` — Create note (`title` 1–200, `location` 1–255, `bodyMd` 100–10000).
...
- **POST** `/plans` — **Create/queue** a plan from a note. Returns a plan in `generationStatus=queued` (and `decisionStatus=generated`).
  - Body:
    ...
    {
      "noteId": "uuid",
      "params": { "days": 7, "travelers": 2, "startDay": "12-11-2025" }, // all optional; days>0? travelers>0? startDay defaulted to now
...
- **POST** `/plans/{planGenerationId}/retry` — Re-queue generation for this plan (creates a new revision internally;).
...
```json
{
  ...
  "decisionStatus": "generated", // notgenerated/generated|accepted|rejected
...
- **GET** `/meta/enums` — Enumerations used by UI:
  ```json
  {
    "planDecisionStatus": ["notGenerated", "generated", "accepted", "rejected"],
...
- **Commands**:
  ...
  - `UpdatePlanContent`
  - `PlanStatusDecision (Accepted)`
  - `PlanStatusDecision (Rejected)`
  - `RetryPlanGeneration`
  - `UpdatePlanContent`
...
| POST `/plans/{id}/accept` | `PlanStatusDecision (Accepted)` |
| POST `/plans/{id}/reject` | `PlanStatusDecision (Rejected)` |
| POST `/plans/{planGenerationId}/retry` | `RetryPlanGeneration` |
...
- **Plans**:
  ...
  - `params.days` > 0 if provided; `params.travelers` > 0 if provided; `params.start_date` to today if not provided,
...
Clarification: Each plan generation request selects exactly one note; users may create multiple plans for the same note over time.
